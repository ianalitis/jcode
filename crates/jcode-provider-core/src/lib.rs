...
fn with_spawn_deadline(stream: EventStream, deadline: tokio::time::Instant) -> EventStream {
    Box::pin(futures::stream::unfold(
        (stream, false),
        move |(mut stream, deadline_emitted)| async move {
            if deadline_emitted {
                return None;
            }
            match tokio::time::timeout_at(deadline, stream.next()).await {
                ...
                Ok(None) => None,
                Err(_) => Some((
                    Err(anyhow::anyhow!("spawn execution deadline exceeded")),
                    (stream, true),
                )),
            }
        },
...
pub trait Provider: Send + Sync {
    ...
    /// Send a request under a spawn-scoped budget, deadline, data-class and
    /// router envelope. Providers that implement metered reservation/settlement
    /// or router policy override this. The default enforces deadlines for
    /// non-metered providers and fails closed instead of ignoring spend controls.
    async fn complete_with_spawn_envelope(
        ...
        let completion = self.complete(messages, tools, system, resume_session_id);
    ...
    /// Split-prompt counterpart to [`Provider::complete_with_spawn_envelope`].
    async fn complete_split_with_spawn_envelope(
        ...
        let dynamic_messages = messages_with_dynamic_system_context(messages, system_dynamic);
        self.complete_with_spawn_envelope(
            &dynamic_messages,
            tools,
            system_static,
            resume_session_id,
    ...
    /// Billing class this provider declares for swarm spawn admission when its
    /// route cannot be classified from session metadata. `None` (the default)
    /// means unknown, and admission fails closed. Only providers that never
    /// leave the process (in-memory test doubles, loopback stubs) should
    /// return `Some(RouteClass::Local)`.
...
#[cfg(test)]
mod tests {
    include!("lib_tests_body_tests.rs");
    include!("spawn_envelope_tests.rs");
}
