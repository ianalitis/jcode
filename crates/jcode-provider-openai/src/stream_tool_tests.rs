...
/// Drain every event that is ready now, stopping at the first Pending.
fn drain_ready(stream: &mut OpenAIResponsesStream) -> Vec<StreamEvent> {
...
/// Per-call view of keyed tool events: (starts with names, joined deltas, ends).
fn keyed_calls(events: &[StreamEvent]) -> HashMap<String, (Vec<String>, String, usize)> {
    let mut calls: HashMap<String, (Vec<String>, String, usize)> = HashMap::new();
    for event in events {
        match event {
            StreamEvent::ToolUseStart { id, name } => {
                calls.entry(id.clone()).or_default().0.push(name.clone())
            }
            StreamEvent::ToolInputDeltaFor { id, delta } => {
                calls.entry(id.clone()).or_default().1.push_str(delta)
            }
            StreamEvent::ToolUseEndFor { id } => calls.entry(id.clone()).or_default().2 += 1,
...
fn late_named_earlier_call_does_not_preempt_an_active_call_with_an_empty_name() {
    // Keyed events let calls interleave, so assert per-call invariants instead
    // of a global order: an empty `name` on a later delta must not rename or
    // restart the active call, and the late-named call still streams once.
    let (tx, mut stream) = stream();
    delta(&tx, "early", "{");
    idle(&mut stream);
    ...
    let events = drain_ready(&mut stream);
    let calls = keyed_calls(&events);
    assert_eq!(calls.len(), 2, "{events:?}");
    let active = &calls["call_active"];
    assert_eq!(active.0, vec!["read".to_string()], "{events:?}");
    assert_eq!(active.1, "{\"ok\":true}");
    assert_eq!(active.2, 1);
    let early = &calls["call_early"];
    assert_eq!(early.0, vec!["ls".to_string()], "{events:?}");
    assert_eq!(early.1, "{\"path\":\"文档\"}");
    assert_eq!(early.2, 1);
    assert!(stream.streaming_tool_calls.is_empty());
}
...
fn missing_or_empty_call_id_uses_item_id_and_finishes_once() {
    for call_id in [None, Some("")] {
        ...
        // A repeated done for a finished item must not start or end it again.
        done(&tx, "item_a", "{}");
        let events = drain_ready(&mut stream);
        let calls = keyed_calls(&events);
        assert_eq!(calls.len(), 1, "{events:?}");
        let call = &calls["item_a"];
        assert_eq!(call.0, vec!["read".to_string()], "{events:?}");
        assert_eq!(call.1, "{}");
        assert_eq!(call.2, 1, "{events:?}");
        assert!(stream.streaming_tool_calls.is_empty());
    }
}
