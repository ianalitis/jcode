...
#[test]
fn fable_5_1_keeps_the_exact_message_prefix_stable() {
    let first = vec![
        ApiMessage {
            role: "user".to_string(),
    ...
    let first_fable = format_messages_for_model(first, true, "claude-fable-5-1");
    let second_fable = format_messages_for_model(second.clone(), true, "claude-fable-5-1");
    let second_fable_1m = format_messages_for_model(second.clone(), true, "claude-fable-5-1[1m]");
    assert_eq!(
        serde_json::to_value(&second_fable_1m).expect("serialize 1m request messages"),
        serde_json::to_value(&second_fable).expect("serialize request messages")
    );
    let opus = format_messages_for_model(second, true, "claude-opus-5");
    let first = serde_json::to_value(first_fable).expect("serialize first request messages");
    let second = serde_json::to_value(second_fable).expect("serialize second request messages");
    let first = first.as_array().expect("first request messages");
    let second = second.as_array().expect("second request messages");

    assert_eq!(&second[..first.len()], first);
    assert!(
        !first
            .iter()
            .any(|message| message.to_string().contains("cache_control"))
    );

    assert!(
        serde_json::to_string(&opus)
            .unwrap()
