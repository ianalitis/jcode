...
impl MultiProvider {
    ...
    async fn complete_with_failover(
        ...
        for candidate in sequence {
            ...
            // Offer a switch only to an alternate that could actually serve the
            // request. A credential-free daemon has none, so it must fall through
            // to the "not configured ... /login" error rather than propose a
            // switch to a provider that is not configured either.
            if candidate != active
                && failover_reason.is_some()
                && self.provider_is_configured(candidate)
            {
...
mod cache_ttl;
pub use cache_ttl::{cache_ttl_for_provider, cache_ttl_for_provider_model, cache_ttl_is_estimate};
