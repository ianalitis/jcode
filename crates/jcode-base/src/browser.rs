...
fn launch_browser_detached(kind: BrowserKind, url: Option<&str>) -> bool {
    // Firefox runs the automation in a dedicated (headless by default) agent
    // profile. Other browsers have no such isolation yet and open the user's
    // own install, so they are only reached by explicit request.
    if kind.family() == BrowserFamily::Gecko {
        return launch_firefox_agent_profile_detached(url);
    }
    #[cfg(target_os = "linux")]
    {
...
pub async fn try_launch_browser_for_bridge_with(
    ...
    // Firefox launches a dedicated agent profile that coexists with a personal
    // Firefox, so a running Firefox does not suppress it. Other browsers launch
    // the user's own install, so keep upstream's "already running" guard there.
    if !should_attempt_firefox_launch(status)
        || (kind.family() != BrowserFamily::Gecko && is_browser_running(kind))
    {
        return Ok(None);
    }
