...
impl App {
    pub(in crate::tui::app) fn poll_ssh_login_onboarding(&mut self) -> bool {
        ...
        let Some(task) = self.remote_login_onboarding.task.as_mut() else {
...
mod tests {
    ...
    #[test]
    fn ssh_onboarding_missing_task_waits_for_runtime_and_failed_status_does_not_retry() {
        with_app(|app| {
            ...
            // The test Target intercepts this status task before any SSH effect.
            assert!(!app.poll_ssh_login_onboarding());
            assert!(!app.remote_login_onboarding.checked);
            assert!(app.remote_login_onboarding.task.is_some());
            assert!(!app.poll_ssh_login_onboarding());
            assert!(app.remote_login_onboarding.checked);
            assert!(app.remote_login_onboarding.task.is_none());
            assert!(!app.poll_ssh_login_onboarding());
