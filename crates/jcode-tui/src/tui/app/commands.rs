...
pub(super) fn handle_feedback_command(app: &mut App, trimmed: &str) -> bool {
    ...
    app.push_display_message(DisplayMessage::system(
        "Feedback uploads were removed from this build. Nothing was sent.".to_string(),
    ));
    app.set_status_notice("Feedback not sent");
    true
}

/// `/telemetry` reports the build's permanent no-collection policy.
pub(super) fn handle_telemetry_command(app: &mut App, trimmed: &str) -> bool {
    ...
    app.push_display_message(DisplayMessage::system(
        "Telemetry, crash reporting, feedback and transcript uploads were removed from this build. They cannot be enabled.".to_string(),
    ));
    app.set_status_notice("Telemetry: off (removed from this build)");
    true
}
