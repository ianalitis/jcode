...
impl Agent {
    pub(super) async fn run_turn_streaming_mpsc(
        ...
        'turn: loop {
            ...
            let mut stream = {
                let spawn_execution_envelope = self.spawn_execution_envelope.clone();
                let complete_future = if let Some(envelope) = spawn_execution_envelope.as_ref() {
                    provider.complete_split_with_spawn_envelope(
                        send_messages,
                        &tools,
                        &split_prompt.static_part,
                ...
                let mut complete_future = std::pin::pin!(complete_future);
                loop {
                    tokio::select! {
                        _ = keepalive.tick() => {
