...
impl Tool for McpCallTool {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        // Re-check after the wait: the session policy may have been revoked (or
        // the fixed surface disabled) while this dispatch was parked on the
        // manager lock. Fail closed on a missing policy before anything else.
        super::checked_session_tool_policy(&ctx)?;
        let names = crate::mcp::dispatch_names(&catalog);
        let alias = catalog
            .iter()
        ...
        let allowed = self
            ...
            .and_then(|r| r.upgrade())
            .map_or_else(
                || super::session_mcp_alias_is_allowed(&ctx, alias, &dispatched_name, "mcp_call"),
                |r| {
                    r.mcp_dispatch_is_allowed(
                        &ctx.session_id,
                        &params.server,
