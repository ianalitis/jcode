...
struct CommunicateInput {
    ...
    #[serde(default)]
    model: Option<String>,
    /// Case-insensitive filter over model id, provider, and auth method for the
    /// `list_models` action. The full catalog is large, so query instead of
    /// dumping every route.
    #[serde(default)]
    query: Option<String>,
        ...
        let mut schema = json!({
            ...
            "properties": {
                ...
                "action": {
                    ...
                    "description": "Action. spawn requires label and should include prompt. list_models lists routes; query filters."
                },
                "key": {
                    "type": "string",
...
include!("communicate_model_list.rs");
