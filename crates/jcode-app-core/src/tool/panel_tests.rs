...
fn context(dir: &std::path::Path, session: &str) -> ToolContext {
    // A production agent turn registers a session policy; the fail-closed
    // dispatch guard requires one for AgentTurn contexts.
    crate::tool::set_session_tool_policy(session, None, std::collections::HashSet::new());
    ToolContext {
