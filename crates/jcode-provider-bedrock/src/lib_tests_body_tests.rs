...
fn isolated_env_vars(home: &std::path::Path) -> Vec<EnvVarGuard> {
    let mut vars = vec![
        EnvVarGuard::set("JCODE_HOME", home.as_os_str()),
        EnvVarGuard::set("XDG_CONFIG_HOME", home.as_os_str()),
        EnvVarGuard::set("AWS_EC2_METADATA_DISABLED", "true"),
    ];
    vars.extend(
        [
...
impl IsolatedTestEnv {
    fn new() -> Self {
        ...
        let vars = isolated_env_vars(temp.path());
        Self {
            _vars: vars,
            _temp: temp,
            _lock: lock,
...
#[test]
fn isolated_test_home_does_not_read_or_mutate_outer_config() {
    let _guard = lock_test_env();
    let outer = tempfile::tempdir().unwrap();
    let outer_config = outer.path().join("config/jcode/bedrock.env");
    std::fs::create_dir_all(outer_config.parent().unwrap()).unwrap();
    let sentinel = format!("{API_KEY_ENV}=outer-file-sentinel\n{REGION_ENV}=outer-region\n");
    std::fs::write(&outer_config, &sentinel).unwrap();
    let _outer_home = EnvVarGuard::set("JCODE_HOME", outer.path().as_os_str());
    let _outer_token = EnvVarGuard::set(API_KEY_ENV, "outer-env-sentinel");
    let _outer_region = EnvVarGuard::set(REGION_ENV, "outer-env-region");
    let _outer_web_identity = EnvVarGuard::set(
        "AWS_WEB_IDENTITY_TOKEN_FILE",
        outer.path().join("outer-token").as_os_str(),
    );

    {
        let temp = tempfile::tempdir().unwrap();
        let _inner_vars = isolated_env_vars(temp.path());
        assert_eq!(BedrockProvider::configured_bearer_token(), None);
        assert_eq!(BedrockProvider::configured_region(), None);
        assert!(!BedrockProvider::has_credentials());
        jcode_provider_env::save_env_value_to_env_file(API_KEY_ENV, ENV_FILE, Some("inner-token"))
            .unwrap();
    }

    assert_eq!(std::fs::read_to_string(&outer_config).unwrap(), sentinel);
    assert_eq!(
        std::env::var_os(API_KEY_ENV).as_deref(),
        Some(OsStr::new("outer-env-sentinel"))
    );
    assert_eq!(
        std::env::var_os(REGION_ENV).as_deref(),
        Some(OsStr::new("outer-env-region"))
    );
    assert_eq!(
        std::env::var_os("AWS_WEB_IDENTITY_TOKEN_FILE").as_deref(),
        Some(outer.path().join("outer-token").as_os_str())
    );
}
