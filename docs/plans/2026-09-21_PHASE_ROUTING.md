...
> **Historical treatment table, not current dispatch instructions.** Phases 1
> and 2 are implemented (`3e410d7d0`, `62008c4ee`). Phase 3 is published as open
> PR #1360, not merged upstream. Phase 4 remains a draft awaiting measurement
> decisions. Recheck current policy, routes, quota and authority before new work.
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 picker family env lock | `opencode-go:glm-5.3-flash` | medium | none | Read, Edit, `cargo test -p jcode-tui`, `git diff`, `git commit --only` | 6 parallel runs, family (b) failures 12 -> 0, wall < 60s/run, guardrails `--skip-slow` | Implemented: `3e410d7d0` |
| 2 flicker history thread-local | `opencode-go:glm-5.3-flash`; escalate `openai-oauth:gpt-5.6-terra` medium if diff leaves one file | medium | `opencode-go:mimo-v2.5` none: list readers of the four frame-metrics statics | Read, Edit, `cargo test -p jcode-tui`, `cargo clippy -p jcode-tui` | 6 parallel runs, 0 failures, single-thread count unchanged, full guardrails | Implemented: `62008c4ee` |
| 3 upstream #1358 port | `openai-oauth:gpt-5.6-terra` | medium | `opencode-go:mimo-v2.5` none: map the 11 tests across both layouts | worktree only; `cargo test -p jcode-base`, `git` | 3 parallel runs 11 -> 0 in a pristine worktree, no new clippy findings, PR body with A/B | Published: #1360; upstream merge pending |
| 4 routing measurement contract | `openai-oauth:gpt-5.3-codex-spark` or `opencode-go:mimo-v2.5` | low | `openai-oauth:gpt-5.6-terra` low: one-question review | Read, Write to one docs file | grep-verifiable checklist, < 600 words, Terra answers "no" | Draft; unresolved measurement decisions |
...
- Any future Phase 3 source writing is serialized with other repository writers,
  even in another worktree. Read-only analysis can overlap within the current cap.
- Phase 4 documentation edits also take the shared writer lease; only read-only
  preparation is eligible to overlap another writer.
- One writer lease covers the repository and all linked worktrees. A worktree
  does not grant a second writer slot. Bound read-only readers by the current
  configured concurrency cap and task packet.
