...
| TUI test isolation regressed: the git probe runs in tests again | [#1365](https://github.com/1jehuang/jcode/issues/1365) | [#1366](https://github.com/1jehuang/jcode/pull/1366) | #1133 was closed as completed but `gather_git_info` still spawns the probe on `master`; restores the test-only read path and takes `jcode-tui --lib` from 19 to 17 failures; re-review 5/5 with independent probe-blocking checks |
| two TUI account tests assert labels `upsert_account` never assigns | [#1367](https://github.com/1jehuang/jcode/issues/1367) | [#1368](https://github.com/1jehuang/jcode/pull/1368) | the store assigns `prefix-<animal>` and ignores the requested label; tests now use the returned label and the switch test uses two accounts so it fails without the switch (19 -> 15 failures with #1366's 2); fresh review on the fixed head reports 0 findings |
| `2026-09-22-session-persist-explicit-state.md` | [#1339](https://github.com/1jehuang/jcode/issues/1339) | [#1373](https://github.com/1jehuang/jcode/pull/1373), from `pr/session-persist-explicit-state` | `Session::save()` drops `is_debug`/`is_canary`/`improve_mode` on a blank session because the exemption list #1144 started was never extended; the `improve_mode` clause is needed for the fourth test and was measured rather than assumed (reverted guard: 3 e2e + 1 TUI failures; all three clauses: 0 and 0; `is_debug`/`is_canary` alone: e2e 0 but TUI still 1) |
