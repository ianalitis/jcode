...
pub(crate) fn run_storage_status_command(json: bool) -> Result<()> {
    ...
    // matched to an installed version; the report is still useful.
    let current_exe = match std::env::current_exe() {
        Ok(path) => Some(path),
        Err(error) => {
            crate::logging::warn(&format!(
                "storage status: could not resolve the current executable: {error}"
            ));
            None
        }
    };
    ...
    match std::fs::symlink_metadata(&versions_dir) {
        Ok(metadata) if metadata.file_type().is_dir() => {
            ...
            for entry in entries {
                ...
                // A version nothing points at has no references, which is
                // exactly what the report should say.
                let mut active_references = Vec::new();
                if let Some(found) = references.remove(&version) {
                    active_references = found;
                }
