...
#[test]
fn prompt_response_preserves_reported_zero_and_saturates_totals() {
    let mut usage = TurnUsage::default();
    usage.add(0, 0, Some(0), None);
    assert_eq!(
        prompt_response("end_turn", &usage),
        json!({
            "stopReason": "end_turn",
    ...
    usage.add(u64::MAX, 1, Some(2), Some(3));
    usage.add(1, 0, None, None);
    assert_eq!(
        prompt_response("cancelled", &usage),
        json!({
            "stopReason": "cancelled",
